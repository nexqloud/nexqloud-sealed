...
func (a *Aggregator) ExcludeOperator(destructionID, operatorID string) error {
	if destructionID == "" {
	...
	if !ok {
		return fmt.Errorf("destruction %q not registered", destructionID)
	}
	if p.Proof != nil {
		return fmt.Errorf("destruction %q already aggregated", destructionID)
	}
	for _, id := range p.Exclusions {
		if id == operatorID {
			return nil
		}
	}
	found := false
	newQuorum := make([]string, 0, len(p.Quorum))
	for _, id := range p.Quorum {
		if id == operatorID {
			found = true
			continue
		}
	...
	if !found {
		return fmt.Errorf("operator %q not in quorum for destruction %q", operatorID, destructionID)
	}
	p.Quorum = newQuorum
	p.Exclusions = append(p.Exclusions, operatorID)
	return nil
}
...
func (a *Aggregator) GetPending(destructionID string) (*PendingDestruction, bool) {
	...
	copy.Exclusions = append([]string(nil), p.Exclusions...)
