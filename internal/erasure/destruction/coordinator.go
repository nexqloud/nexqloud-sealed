...
func (c *Coordinator) CreateDestruction(ctx context.Context, req CreateDestructionRequest) (Session, error) {
	...
	overallOK := true
	activeQuorum := append([]string(nil), quorum...)
	for _, opID := range quorum {
		result := c.dispatchToOperator(ctx, session, opID, req.CustomerSig)
		if result.Status == "failed" && isUnreachableDetail(result.Detail) {
			result = c.handleUnreachableDispatch(ctx, session, opID, req.CustomerSig, result)
			if result.Status == "erased_by_exclusion" {
				activeQuorum = removeOperator(activeQuorum, opID)
			}
		}
		dispatches = append(dispatches, result)
		if result.Status != "dispatched" && result.Status != "erased_by_exclusion" {
			overallOK = false
		}
	}
	session.Quorum = activeQuorum
...
func (c *Coordinator) handleUnreachableDispatch(ctx context.Context, session Session, operatorID string, customerSig []byte, prior DispatchResult) DispatchResult {
	reach := func(ctx context.Context, opID, _ string) bool {
		r := c.dispatchToOperator(ctx, session, opID, customerSig)
		return r.Status == "dispatched"
	}
	handler := c.FailureHandler
	if handler == nil {
		handler = NewFailureHandler(c.CoordinatorSK, reach)
	} else if handler.Reach == nil {
		handler.Reach = reach
	}
	outcome := handler.HandleUnreachable(ctx, operatorID, session.TenantID)
	switch outcome {
	case OutcomeDestroyed:
		return DispatchResult{OperatorID: operatorID, Status: "dispatched"}
	case OutcomeErasedByExclusion:
		if err := c.notifyAggregatorExclusion(ctx, session.DestructionID, operatorID); err != nil {
			return DispatchResult{
				OperatorID: operatorID,
				Status:     "failed",
				Detail:     prior.Detail + "; exclusion logged but aggregator notify failed: " + err.Error(),
			}
		}
		return DispatchResult{
			OperatorID: operatorID,
			Status:     "erased_by_exclusion",
			Detail:     "operator unreachable; key slot erased by exclusion (cryptographic erasure, not physical wipe)",
		}
	default:
		return prior
	}
...
func (c *Coordinator) notifyAggregatorExclusion(ctx context.Context, destructionID, operatorID string) error {
...
func removeOperator(quorum []string, operatorID string) []string {
	out := make([]string, 0, len(quorum))
