...
export async function onRequestGet(context) {
  ...
  const kind = "sealed-model-attest-issuers";
  ...
  try {
    const local = new URL(`/sealed-model-attest-issuers/${env}/allowlist.json`, url.origin);
  ...
  return new Response(
    JSON.stringify({ error: "model attest issuers allowlist not found", env }),
    { status: 404, headers },
  );
}
