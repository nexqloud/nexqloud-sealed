...
export async function onRequestGet(context) {
  ...
  const kind = "sealed-wipe-issuers";
  ...
  try {
    const local = new URL(`/sealed-wipe-issuers/${env}/allowlist.json`, url.origin);
  ...
  return new Response(
    JSON.stringify({ error: "wipe issuers allowlist not found", env }),
    { status: 404, headers },
  );
}
