...
func main() {
	...
	tenantID := flag.String("tenant", "acme", "tenant id for the commitment record")
	keyVersion := flag.Int("key-version", 1, "tenant key version")
	...
	fmt.Printf("posted %s record (key_version=%d) with wraps for %s\n", record.TenantID, record.KeyVersion, strings.Join(sortedKeys(wraps), ", "))
