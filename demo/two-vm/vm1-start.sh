...
start_service_or_die registry "$(build_demo_bin registry ./demo/registry/main.go)"
...
start_service_or_die mock-idp "$(build_demo_bin mock-idp ./demo/mock-idp/main.go)" \
...
if curl -fsS "$REGISTRY_LOCAL/records/${TENANT_ID}" >/dev/null 2>&1; then
  ...
  "$(build_demo_bin bootstrap ./demo/bootstrap/main.go)" \
...
start_service_or_die operator-a "$(build_demo_bin operator ./cmd/operator/main.go)" \
